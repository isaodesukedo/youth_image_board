...
const inp = { width: '100%', background: '#292524', color: 'white', borderRadius: '12px', padding: '10px 16px', border: '1px solid #57534e', outline: 'none', fontSize: '0.875rem', boxSizing: 'border-box' as const }
...
export default function AddImageModal({ spaceId, onClose, onAdded }: { spaceId: string; onClose: () => void; onAdded: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.82)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: '#1c1917', borderRadius: '16px 16px 0 0', width: '100%', maxWidth: '400px', border: '1px solid #44403c', maxHeight: '92vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #292524', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: '#1c1917' }}>
          <h2 style={{ fontWeight: 'bold', color: 'white', margin: 0 }}>イメージを追加</h2>
          <button style={{ background: 'none', border: 'none', color: '#a8a29e', fontSize: '1.5rem', cursor: 'pointer' }} onClick={onClose}>×</button>
        </div>
        <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div onClick={() => fileRef.current?.click()} style={{
            aspectRatio: '16/9', borderRadius: '12px', border: '2px dashed #57534e',
            display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
            background: '#292524', overflow: 'hidden'
          }}>
            {preview
              ? <img src={preview} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '2rem', marginBottom: '4px' }}>📷</div>
                  <p style={{ color: '#a8a29e', fontSize: '0.875rem', margin: 0 }}>タップして画像を選択</p>
                </div>
            }
          </div>
          ...
          <div style={{ display: 'flex', gap: '8px', paddingBottom: '8px' }}>
            <button onClick={onClose} style={{ flex: 1, padding: '12px', borderRadius: '12px', background: 'transparent', color: 'white', border: '1px solid #57534e', cursor: 'pointer', fontSize: '0.875rem' }}>キャンセル</button>
            <button onClick={handleUpload} disabled={!file || loading} style={{ flex: 1, padding: '12px', borderRadius: '12px', background: loading || !file ? '#4c1d95' : '#7c3aed', color: 'white', border: 'none', cursor: file && !loading ? 'pointer' : 'default', fontSize: '0.875rem', fontWeight: '600', opacity: !file ? 0.5 : 1 }}>
