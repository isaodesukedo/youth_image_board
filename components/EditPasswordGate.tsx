...
export default function EditPasswordGate({ onSuccess, onCancel }: { onSuccess: () => void; onCancel: () => void }) {
  const [input, setInput] = useState('')
  const [error, setError] = useState(false)

  useEffect(() => {
    // すでに認証済みならそのまま通す
    if (sessionStorage.getItem('edit_authed') === 'true') { onSuccess() }
  }, [])

  const handleSubmit = () => {
    if (input === process.env.NEXT_PUBLIC_SITE_PASSWORD) {
      sessionStorage.setItem('edit_authed', 'true')
      onSuccess()
    } else {
      setError(true)
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={onCancel}>
      <div style={{ background: 'white', borderRadius: 20, padding: 28, width: '100%', maxWidth: 320, boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }} onClick={e => e.stopPropagation()}>
        ...
        <h2 style={{ fontSize: 18, fontWeight: 700, color: '#1e293b', textAlign: 'center', margin: '0 0 6px' }}>編集モード</h2>
        <p style={{ color: '#64748b', fontSize: 13, textAlign: 'center', margin: '0 0 20px' }}>編集するにはパスワードが必要です</p>
        <input
          ...
          style={{
            ...
            padding: '12px 16px', marginBottom: 12, border: `2px solid ${error ? '#ef4444' : '#e2e8f0'}`,
            outline: 'none', fontSize: 16, boxSizing: 'border-box', fontFamily: 'inherit',
            transition: 'border-color 0.2s'
          }}
        />
        {error && <p style={{ color: '#ef4444', fontSize: 13, margin: '0 0 12px', textAlign: 'center' }}>パスワードが違います</p>}
