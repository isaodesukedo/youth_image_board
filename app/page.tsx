...
export default function Home() {
  ...
  const getState = (): LayoutState => ({ areas1F, areas2F, areas3F, syms1F, syms2F, syms3F, lines1F, lines2F, lines3F })
  ...
  const applyState = (state: LayoutState) => {
  ...
  const undo = () => { if (historyIdxRef.current <= 0) return; historyIdxRef.current--; applyState(historyRef.current[historyIdxRef.current]) }
  const redo = () => { if (historyIdxRef.current >= historyRef.current.length - 1) return; historyIdxRef.current++; applyState(historyRef.current[historyIdxRef.current]) }

  useEffect(() => { if (!skipHistoryRef.current) pushHistory(getState()) }, [areas1F, areas2F, areas3F, syms1F, syms2F, syms3F, lines1F, lines2F, lines3F])
  ...
  const requestEdit = (floor: 1|2|3) => {
    if (sessionStorage.getItem('edit_authed') === 'true') {
  ...
  const loadLayout = async () => {
    ...
    if (data?.data) {
      ...
      if (d.lines1F) setLines1F(d.lines1F)
      if (d.lines2F) setLines2F(d.lines2F)
      if (d.lines3F) setLines3F(d.lines3F)
  ...
  const handleAreaNameChange = async (oldName: string, newName: string) => {
    ...
    await supabase.from('spaces').update({ name: newName }).eq('name', oldName)
    load()
  }
  const handleAreaAdd = async (key: string, floor: number) => {
    await supabase.from('spaces').insert({ name: key, floor, color: '#6b7280' })
    load()
  }
  const handleAreaDelete = async (key: string) => {
    if (!key) return
    await supabase.from('spaces').delete().eq('name', key)
    load()
  }
  ...
  const editorProps = { spaces, counts, onSpaceClick: openSpace, onAreaNameChange: handleAreaNameChange, onAreaAdd: handleAreaAdd, onAreaDelete: handleAreaDelete }
      ...
      <main style={{ minHeight: '100vh', background: '#f8fafc', color: '#1e293b', paddingBottom: 80 }}>
        ...
        <div style={{ background: 'white', borderBottom: '1px solid #e2e8f0', padding: '16px 20px', position: 'sticky', top: 0, zIndex: 30 }}>
          <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 800, margin: 0, background: 'linear-gradient(135deg, #7c3aed, #f97316)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                🏠 場作りボード
              </h1>
              <p style={{ color: '#94a3b8', fontSize: 12, margin: '2px 0 0' }}>みんなでイメージを共創しよう</p>
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={undo} style={{ background: '#f1f5f9', border: 'none', color: '#64748b', borderRadius: 8, padding: '6px 10px', fontSize: 12, cursor: 'pointer' }}>↩</button>
              <button onClick={redo} style={{ background: '#f1f5f9', border: 'none', color: '#64748b', borderRadius: 8, padding: '6px 10px', fontSize: 12, cursor: 'pointer' }}>↪</button>
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px' }}>
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: 14, marginBottom: 16, border: '1px solid #e2e8f0', boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
            ...
            <img src="https://ogjcaupnyokrzgunwfbg.supabase.co/storage/v1/object/public/youth_image/floorplan.png"
              alt="図面" style={{ width: '100%', borderRadius: 10, display: 'block' }}
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: 14, marginBottom: 24, border: '1px solid #e2e8f0', boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
            ...
            <FloorPlanEditor {...editorProps} floorId="1f" floor={1} floorLabel="1F オープン空間"
              areas={areas1F} setAreas={setAreas1F} lines={lines1F} setLines={setLines1F} symbols={syms1F} setSymbols={setSyms1F}
              editMode={editMode1F} setEditMode={v => v ? requestEdit(1) : handleEditDone(1)} viewBox="0 0 80 82"/>
            <FloorPlanEditor {...editorProps} floorId="2f" floor={2} floorLabel="2F 静かな空間"
              areas={areas2F} setAreas={setAreas2F} lines={lines2F} setLines={setLines2F} symbols={syms2F} setSymbols={setSyms2F}
              editMode={editMode2F} setEditMode={v => v ? requestEdit(2) : handleEditDone(2)} viewBox="0 0 66 76"/>
            <FloorPlanEditor {...editorProps} floorId="3f" floor={3} floorLabel="3F 事務所"
              areas={areas3F} setAreas={setAreas3F} lines={lines3F} setLines={setLines3F} symbols={syms3F} setSymbols={setSyms3F}
              editMode={editMode3F} setEditMode={v => v ? requestEdit(3) : handleEditDone(3)} viewBox="0 0 72 70"/>
          </div>

          {/* スペース管理リスト */}
          <p style={{ color: '#94a3b8', fontSize: 11, fontWeight: 600, marginBottom: 14, textTransform: 'uppercase', letterSpacing: '0.05em' }}>📋 スペース管理リスト</p>
          {[1,2,3].map(floor => {
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
                  {fs.map(space => (
                    <div key={space.id} style={{ position: 'relative' }}>
                      <button onClick={() => openSpace(space)} style={{
                        width: '100%', borderRadius: 14, padding: 14, textAlign: 'left', cursor: 'pointer',
                        background: 'white', border: `2px solid ${space.color}40`,
                        boxShadow: `0 2px 8px ${space.color}20`, color: '#1e293b',
                        ...
                        {space.description && <div style={{ color: '#94a3b8', fontSize: 11, marginTop: 4, lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{space.description}</div>}
                        <div style={{ color: space.color, fontSize: 12, marginTop: 6, fontWeight: 600 }}>{counts[space.id] ? `📷 ${counts[space.id]}枚` : '画像なし'}</div>
                      </button>
                      <button onClick={e => { e.stopPropagation(); setEditSpace(space) }} style={{
                        position: 'absolute', top: 8, right: 8, background: '#f1f5f9', border: 'none',
                        color: '#94a3b8', borderRadius: 8, width: 26, height: 26, cursor: 'pointer',
          ...
          <button onClick={() => setAddSpaceOpen(true)} style={{
            width: '100%', background: 'white', border: '2px dashed #cbd5e1', color: '#94a3b8',
            borderRadius: 14, padding: 16, cursor: 'pointer', fontSize: 14, fontWeight: 600,
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', zIndex: 40, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={() => setSelectedSpace(null)}>
            <div style={{ background: 'white', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: 640, maxHeight: '88vh', display: 'flex', flexDirection: 'column', boxShadow: '0 -8px 32px rgba(0,0,0,0.15)' }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {selectedSpace.description && (
                      <p style={{ color: '#64748b', fontSize: 13, margin: '0 0 0 24px', lineHeight: 1.6 }}>{selectedSpace.description}</p>
                    )}
                  </div>
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 8 }}>
                    <button onClick={() => { setEditSpace(selectedSpace); setSelectedSpace(null) }}
                      style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', color: '#64748b', borderRadius: 10, padding: '6px 10px', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>
                      ✏️ 編集
                    </button>
              ...
              <div style={{ padding: '12px 20px', borderBottom: '1px solid #f1f5f9', flexShrink: 0 }}>
                <button onClick={() => setAddImageOpen(true)} style={{
                  width: '100%', background: `${selectedSpace.color}15`, border: `2px dashed ${selectedSpace.color}60`,
                  color: selectedSpace.color, borderRadius: 12, padding: 12, cursor: 'pointer', fontSize: 14,
              ...
              <div style={{ overflowY: 'auto', padding: 16, flex: 1 }}>
                {loadingImages ? (
                  ...
                  <div style={{ textAlign: 'center', padding: 40, color: '#cbd5e1' }}>
                    ...
                    <p style={{ fontSize: 12, color: '#cbd5e1' }}>上のボタンから追加してみよう</p>
                  </div>
                ) : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
                    {spaceImages.map(img => (
                      <div key={img.id} onClick={() => setDetailImage(img)}
                        style={{ position: 'relative', aspectRatio: '1', borderRadius: 12, overflow: 'hidden', cursor: 'pointer', border: '1px solid #f1f5f9', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
                        <img src={img.public_url} alt={img.title || ''} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        {img.title && (
                          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.75), transparent)', padding: '18px 8px 8px' }}>
                            <p style={{ margin: 0, fontSize: 12, color: 'white', fontWeight: 600, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{img.title}</p>
                          </div>
                        )}
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setDetailImage(null)}>
            <div style={{ background: 'white', borderRadius: 20, width: '100%', maxWidth: 400, overflow: 'hidden', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 16px 48px rgba(0,0,0,0.3)' }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ padding: 20 }}>
                ...
                {detailImage.reason && (
                  <div style={{ marginBottom: 10, background: '#f5f3ff', borderRadius: 12, padding: 14, border: '1px solid #e9d5ff' }}>
                    <div style={{ fontSize: 12, color: '#7c3aed', fontWeight: 700, marginBottom: 4 }}>✨ なぜいいか</div>
                    <p style={{ margin: 0, fontSize: 14, color: '#4c1d95', lineHeight: 1.6 }}>{detailImage.reason}</p>
                  </div>
                )}
                {detailImage.experience && (
                  <div style={{ marginBottom: 10, background: '#ecfdf5', borderRadius: 12, padding: 14, border: '1px solid #a7f3d0' }}>
                    <div style={{ fontSize: 12, color: '#059669', fontWeight: 700, marginBottom: 4 }}>🌟 どんな体験ができるか</div>
                    <p style={{ margin: 0, fontSize: 14, color: '#065f46', lineHeight: 1.6 }}>{detailImage.experience}</p>
                  </div>
                )}
        ...
        {editPasswordPending && (
          <EditPasswordGate
            ...
            onCancel={() => setEditPasswordPending(null)}
          />
        )}
