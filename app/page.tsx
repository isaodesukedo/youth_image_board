...
export default function Home() {
  ...
  const pushHistory = useCallback((state: LayoutState) => {
    ...
    h.push(state); if (h.length > MAX_HISTORY) h.shift()
  ...
  // スペース編集保存後にマップの色・名前も即時反映
  const handleSpaceSaved = () => { load() }
      ...
      <main style={{ minHeight: '100vh', background: '#f8fafc', color: '#1e293b', paddingBottom: 80 }}>
        ...
        <div style={{ background: 'white', borderBottom: '1px solid #e2e8f0', padding: '16px 20px', position: 'sticky', top: 0, zIndex: 30 }}>
          <div style={{ maxWidth: 720, margin: '0 auto' }}>
            <h1 style={{ fontSize: 20, fontWeight: 800, margin: 0, background: 'linear-gradient(135deg, #7c3aed, #f97316)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>🏠 場作りボード</h1>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
                  {fs.map(space => (
                    <div key={space.id} style={{ position: 'relative' }}>
                      ...
                      <button onClick={e => { e.stopPropagation(); setEditSpace(space) }} style={{ position: 'absolute', top: 8, right: 8, background: '#f1f5f9', border: 'none', color: '#94a3b8', borderRadius: 8, width: 26, height: 26, cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✏️</button>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <button onClick={() => setAddSpaceOpen(true)} style={{ width: '100%', background: 'white', border: '2px dashed #cbd5e1', color: '#94a3b8', borderRadius: 14, padding: 16, cursor: 'pointer', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              ...
              <div style={{ padding: '8px 20px 14px', borderBottom: '1px solid #f1f5f9' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 8 }}>
                    <button onClick={() => { setEditSpace(selectedSpace); setSelectedSpace(null) }} style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', color: '#64748b', borderRadius: 10, padding: '6px 10px', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>✏️ 編集</button>
            ...
            <div style={{ background: 'white', borderRadius: 20, width: '100%', maxWidth: 400, overflow: 'hidden', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 16px 48px rgba(0,0,0,0.3)' }} onClick={e => e.stopPropagation()}>
              <div style={{ position: 'relative' }}>
                ...
                <button onClick={() => { setEditImage(detailImage); setDetailImage(null) }}
                  style={{ position: 'absolute', top: 10, right: 10, background: 'rgba(255,255,255,0.9)', border: 'none', color: '#64748b', borderRadius: 8, padding: '5px 10px', fontSize: 12, cursor: 'pointer', fontWeight: 600, backdropFilter: 'blur(4px)' }}>
                  ✏️ 編集
                </button>
              </div>
